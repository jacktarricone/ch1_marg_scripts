...
plot_dem_vs_dom <-function(df, bins, scale, title){
  
  plot <-ggplot() +
    geom_tile(data = mean_ez_df, aes(x = elevation, y = dom_dowy), color = 'grey', fill = 'grey', width = 22, height = 2) +
    geom_bin2d(data = df, bins = bins, aes(x = elevation, y= dom_dowy, fill = ..density..)) +
    ...
    annotate(geom="text", y=240, x=3600, label= title, size = 8, fontface = "bold")+
...
# plot
ez_n_plot <-plot_dem_vs_dom(df = ez_n_df,
                             bins = 80,
                             scale = scale1,
                             title = "North Facing") 
# save
ggsave(ez_n_plot,
       file = "./plots/dom_vs_dem_north_plot_v1.png",
       width = 6,
       height = 5,
       dpi = 600)

system("open ./plots/dom_vs_dem_north_plot_v1.png")
