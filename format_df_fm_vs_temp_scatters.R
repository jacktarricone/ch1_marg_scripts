...
# add max SWE and shortwave in future plz
# jack tarricone
# june 8th, 2023
...
fm_stack_v2 <-c(dem_6b,aspect_ns,fm_stack)
temp_stack_v2 <-c(dem_6b,aspect_ns,temp_stack)

# convert to df
df_fm_v1 <-as.data.frame(fm_stack_v2, xy = TRUE, cells = TRUE, na.rm = TRUE)
df_temp_v1 <-as.data.frame(temp_stack_v2, xy = TRUE, cells = TRUE, na.rm = TRUE)
...
# fm
colnames(df_fm_v1)[6:ncol(df_fm_v1)] <-years
colnames(df_fm_v1)[4] <-"ele_bin"

# temp
colnames(df_temp_v1)[6:ncol(df_temp_v1)] <-years
colnames(df_temp_v1)[4] <-"ele_bin"
...
fm_long_df <-as.data.frame((df_fm_v1) %>%
                          pivot_longer(-c(cell,x,y,ele_bin,aspect), names_to = "year", values_to = "frac_melt"))

temp_long_df <-as.data.frame((df_temp_v1) %>%
                             pivot_longer(-c(cell,x,y,ele_bin,aspect), names_to = "year", values_to = "temp_c"))

# join together
full_df <-full_join(fm_long_df, temp_long_df)
full_df_v2 <-na.omit(full_df)
head(full_df_v2)

# save
fwrite(full_df_v2, "./csvs/fm_temp_eb_ns_csv_v4.csv")
