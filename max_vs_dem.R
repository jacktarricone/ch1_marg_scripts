...
plot_dem_vs_max <-function(df, bins, scale, title){
  
  plot <-ggplot() +
    geom_tile(data = mean_ez_df, aes(x = elevation, y = max_swe_m), color = 'grey', fill = 'grey', width = 22, height = .04) +
    geom_bin2d(data = df, bins = bins, aes(x = elevation, y= max_swe_m, fill = ..density..)) +
    scale_fill_gradientn(colors = scale) +
    scale_x_continuous(limits = c(1500,4300), expand = (c(0,0))) +
    scale_y_continuous(limits = c(0,3),expand = (c(0,0))) +
    labs(y = "Max SWE (m)", x = "Elevation (m)")+
    annotate(geom="text", y=2.8, x=3600, label= title, size = 8, fontface = "bold")+
...
# plot
ez_n_plot <-plot_dem_vs_max(df = ez_n_df,
                             bins = 80,
                             scale = scale1,
                             title = "North Facing") 
# save
ggsave(ez_n_plot,
       file = "./plots/max_vs_dem_north_plot_v1.png",
       width = 6,
       height = 5,
       dpi = 600)

system("open ./plots/max_vs_dem_north_plot_v1.png")

# plot
ez_s_plot <-plot_dem_vs_max(df = ez_s_df,
                            bins = 80,
                            scale = scale1,
                            title = "South Facing") 
...
# save
ggsave(ez_s_plot,
       file = "./plots/max_vs_dem_south_plot_v1.png",
       width = 6,
       height = 5,
       dpi = 600)

system("open ./plots/max_vs_dem_south_plot_v1.png")
...
# save
ggsave(n_v_s,
       file = "./plots/max_dem_ns_v2.png",
       width = 11.5, 
       height = 5,
       dpi = 600)

system("open ./plots/max_dem_ns_v2.png")
